...
describe Screen::GuestInfoPlayer do
  it 'guest info player' do
    ...
    worlds_lines = client.get('http://br.twstats.com/').search('.r1,.r2').select{|a| !a.text.include?('closed')}
    worlds_lines = worlds_lines.map do |world_line|
      parts = world_line.text.downcase.split("\n")
      parts.map(&:strip).uniq - ['']
    end

    most_village_world = worlds_lines.sort{|a,b| a.last.number_part <=> b.last.number_part }.last.first
