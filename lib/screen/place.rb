...
class Screen::Place < Screen::Base
  ...
  def parse(page)
    ...
    self.troops = Troop.new(self.troops)
    self.form = page.form
  ...
  def parse_commands(page)
    parse_table(page,'#commands_outgoings').map do |tr|
      ...
      command.returning = tr.search('img[src*=return]').size > 0 || tr.search('img[src*=cancel]').size > 0
  ...
  def send_attack(target,troops)
    ...
    self.form.fields.map do |field|
      ...
      unless current.nil?
        self.form[field.name] = current.to_s
      end
    end
    self.form['x'] = target.x
    self.form['y'] = target.y
    confirm_page = @client.submit(self.form,self.form.buttons.first)
